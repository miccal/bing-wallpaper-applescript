...
* `uhd` can take the values `0` and `1`, with `0` meaning fetch the standard resolution version of the image (normally 1920x1080) and `1` meaning fetch the ultra high definition resolution version of the image (which can vary -- for example, the image obtained in the example below is 5349×3009); 
